...
from configuration import parse_evaluation_arguments, parse_reclusterer_arguments, parse_train_arguments
...
def compute_clustering_matrix():
    pass

def compute_new_candidate_labels():
    pass

def compute_unused_labels():
    pass
class Reclusterer():
    def __init__(self):
        pass
    def get_matching_matrix(cls, xlinear_model, Xt, beam_size=10):
    ...
    def get_topk_clusters(cls, xlinear_model, Xt, Yt, beam_size, n_copies):
        # Get the clustering matrix in the last level
        model_chain = xlinear_model.model.model_chain
        leaf_model = model_chain[-1]

        # Get the matching matrix
        M = cls.get_matching_matrix(xlinear_model, Xt, beam_size=beam_size)
        C = leaf_model.pC.buf
    ...
    def reorganize(cls, args):
        ...
        C, Yt, mapper, unused_labels = cls.get_topk_clusters(
            xlinear_model, Xt, Yt, args.beam_size, args.n_copies
        )
